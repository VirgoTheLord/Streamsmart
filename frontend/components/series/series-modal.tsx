...
import { X, Play, Star, Calendar, Clock, Tv } from "lucide-react";
import Image from "next/image";
import { useEffect, useState } from "react";

interface SeriesModalProps {
  series: TVShow | TVShowDetails | null;
  isOpen: boolean;
  onClose: () => void;
  onWatchSeries?: (seriesId: number, title?: string) => void;
}

export function SeriesModal({ series, isOpen, onClose, onWatchSeries }: SeriesModalProps) {
  ...
  const details = series as TVShowDetails;

  const backdropUrl = series.backdrop_path
    ? `${process.env.NEXT_PUBLIC_TMDB_IMAGE_BASE_URL}/original${series.backdrop_path}`
  ...
  const handleWatchSeries = () => {
    if (onWatchSeries) {
      onWatchSeries(series.id, series.name);
    }
  };
